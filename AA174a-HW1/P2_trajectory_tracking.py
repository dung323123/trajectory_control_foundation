...
class TrajectoryTracker:
    ...
    def compute_control(self, x: float, y: float, th: float, t: float) -> T.Tuple[float, float]:
        ...
        # Virtual control (Eq. 5)
        u1 = xdd_d + self.kpx*(x_d - x) + self.kdx*(xd_d - xd)
        u2 = ydd_d + self.kpy*(y_d - y) + self.kdy*(yd_d - yd)

        # Solve for [V_dot, omega]
        A = np.array([
            [np.cos(th), -V_curr*np.sin(th)],
            [np.sin(th),  V_curr*np.cos(th)]
        ])

        b = np.array([u1, u2])

        V_dot, om = np.linalg.solve(A, b)

        # Integrate V_dot to get V
        V = V_curr + V_dot * dt
